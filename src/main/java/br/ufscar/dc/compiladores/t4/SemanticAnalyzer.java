...
import static br.ufscar.dc.compiladores.t4.SemanticUtils.verifyType;
import static br.ufscar.dc.compiladores.t4.SemanticUtils.addSemanticErrors;
import static br.ufscar.dc.compiladores.t4.SemanticUtils.verificaCompatibilidade;
import static br.ufscar.dc.compiladores.t4.SemanticUtils.mapType;
import br.ufscar.dc.compiladores.t4.SymbolTable.EntryType;
...
public class SemanticAnalyzer extends LinguagemAlgoritmicaBaseVisitor <Void> {
    ...
    static HashMap<String, ArrayList<TypeT4>> functionsAndProcedures = new HashMap<>();
    ...
    public void addTableSymbol(String name, String tipo, Token tokenName, Token TypeT, EntryType TypeE) {
        ...
        switch (tipo) {
            case "literal":
                itemType = TypeT4.LITERAL;
                break;
            case "inteiro":
                itemType = TypeT4.INTEGER;
                break;
            case "real":
                itemType = TypeT4.REAL;
                break;
            case "logico":
                itemType = TypeT4.LOGIC;
                break;
            case "void":
                itemType = TypeT4.VOID;
                break;
            case "registro":
                itemType = TypeT4.REGISTER;
                break;
            default:
                itemType = TypeT4.INVALID;
                break;
        }

        //tipo não existente, retorna erro de não dclarado
        if (itemType == TypeT4.INVALID)
    ...
    public Void visitDeclaracao_global(LinguagemAlgoritmicaParser.Declaracao_globalContext ctx) {
        ...
        ArrayList<TypeT4> variablesTypes = new ArrayList<>();
    ...
    public Void visitCmdEnquanto(LinguagemAlgoritmicaParser.CmdEnquantoContext ctx) {
        ...
        TypeT4 tipo = verifyType(tabela, ctx.expressao());
    ...
    public Void visitCmdAtribuicao(LinguagemAlgoritmicaParser.CmdAtribuicaoContext ctx) {
        ...
        TypeT4 tipoExpressao = verifyType(tabela, ctx.expressao());

        String varNome = ctx.identificador().getText();

        if (tipoExpressao != TypeT4.INVALID) {
            ...
            if (!tabela.exists(varNome))
                ...
                TypeT4 varTipo = verifyType(tabela, varNome);
                if (varTipo == TypeT4.INTEGER || varTipo == TypeT4.REAL) {
                    if (ctx.getText().contains("ponteiro")) {
                        if (!verificaCompatibilidade(varTipo, tipoExpressao))
                            if (tipoExpressao != TypeT4.INTEGER)
